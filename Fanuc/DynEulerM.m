...
%Besides, we also concern about friction: viscous + coloumb friction
%@Input:
%DH: DH parameters
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Torque = DynEulerM(DH,Ma,Cen,Ie,Imz,Fv,Fs,Zm,k,Ft,Pos,Vel,Acc)
...
% consider friction
Tau(1) = Tau(1) + Fv(1)*vel1 + Fs(1)*sign(vel1);
Tau(2) = Tau(2) + Fv(2)*vel2 + Fs(2)*sign(vel2);
Tau(3) = Tau(3) + Fv(3)*vel3 + Fs(3)*sign(vel3);
Tau(4) = Tau(4) + Fv(4)*vel4 + Fs(4)*sign(vel4);
Tau(5) = Tau(5) + Fv(5)*vel5 + Fs(5)*sign(vel5);
Tau(6) = Tau(6) + Fv(6)*vel6 + Fs(6)*sign(vel6);
