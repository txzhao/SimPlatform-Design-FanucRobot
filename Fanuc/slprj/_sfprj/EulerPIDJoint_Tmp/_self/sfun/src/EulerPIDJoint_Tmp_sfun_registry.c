...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_EulerPIDJoint_Tmp_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_EulerPIDJoint_Tmp_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_EulerPIDJoint_Tmp_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_EulerPIDJoint_Tmp_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_EulerPIDJoint_Tmp_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "EulerPIDJoint_Tmp") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_EulerPIDJoint_Tmp_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_EulerPIDJoint_Tmp_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"EulerPIDJoint_Tmp")) {
    return(sf_EulerPIDJoint_Tmp_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void EulerPIDJoint_Tmp_terminator(void);
...
extern void EulerPIDJoint_Tmp_initializer(void);
extern void EulerPIDJoint_Tmp_register_exported_symbols(SimStruct* S);
extern void EulerPIDJoint_Tmp_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  EulerPIDJoint_Tmp_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      EulerPIDJoint_Tmp_debug_initialize();
    }

    EulerPIDJoint_Tmp_initializer();
  }
