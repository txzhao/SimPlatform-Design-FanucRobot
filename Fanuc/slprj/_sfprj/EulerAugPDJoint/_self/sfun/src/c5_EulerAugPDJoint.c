...
static void initialize_c5_EulerAugPDJoint(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance);
static void initialize_params_c5_EulerAugPDJoint
  (SFc5_EulerAugPDJointInstanceStruct *chartInstance);
static void enable_c5_EulerAugPDJoint(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance);
static void disable_c5_EulerAugPDJoint(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance);
static void c5_update_debugger_state_c5_EulerAugPDJoint
  (SFc5_EulerAugPDJointInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c5_EulerAugPDJoint
  (SFc5_EulerAugPDJointInstanceStruct *chartInstance);
static void set_sim_state_c5_EulerAugPDJoint(SFc5_EulerAugPDJointInstanceStruct *
  chartInstance, const mxArray *c5_st);
static void finalize_c5_EulerAugPDJoint(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance);
static void sf_c5_EulerAugPDJoint(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance);
static void compInitSubchartSimstructsFcn_c5_EulerAugPDJoint
  (SFc5_EulerAugPDJointInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c5_machineNumber, uint32_T
  c5_chartNumber);
static void c5_N_d(SFc5_EulerAugPDJointInstanceStruct *chartInstance, real_T
...
static void c5_emlrt_marshallIn(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance, const mxArray *c5_N, const char_T *c5_name, real_T c5_y[6]);
static uint8_T c5_b_emlrt_marshallIn(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance, const mxArray *c5_b_is_active_c5_EulerAugPDJoint, const char_T
  *c5_name);
static void init_dsm_address_info(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c5_is_active_c5_EulerAugPDJoint = 0U;
}
  ...
  (SFc5_EulerAugPDJointInstanceStruct *chartInstance)
{
  ...
  c5_b_hoistedGlobal = chartInstance->c5_is_active_c5_EulerAugPDJoint;
  ...
  chartInstance, const mxArray *c5_st)
{
  ...
  chartInstance->c5_is_active_c5_EulerAugPDJoint = c5_b_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c5_u, 1)),
     "is_active_c5_EulerAugPDJoint");
  sf_mex_destroy(&c5_u);
  c5_update_debugger_state_c5_EulerAugPDJoint(chartInstance);
...
static const mxArray *c5_sf_marshall(void *chartInstanceVoid, void *c5_u)
{
  ...
  const mxArray *c5_b_y = NULL;
  SFc5_EulerAugPDJointInstanceStruct *chartInstance;
  chartInstance = (SFc5_EulerAugPDJointInstanceStruct *)chartInstanceVoid;
  c5_y = NULL;
...
const mxArray *sf_c5_EulerAugPDJoint_get_eml_resolved_functions_info(void)
{
...
static uint8_T c5_b_emlrt_marshallIn(SFc5_EulerAugPDJointInstanceStruct
  *chartInstance, const mxArray *
  c5_b_is_active_c5_EulerAugPDJoint, const char_T *c5_name)
{
  uint8_T c5_y;
  uint8_T c5_u0;
  sf_mex_import(c5_name, sf_mex_dup(c5_b_is_active_c5_EulerAugPDJoint), &c5_u0,
                1, 3, 0U, 0, 0U, 0);
  c5_y = c5_u0;
  sf_mex_destroy(&c5_b_is_active_c5_EulerAugPDJoint);
  return c5_y;
}
...
/* SFunction Glue Code */
void sf_c5_EulerAugPDJoint_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c5_EulerAugPDJoint_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c5_EulerAugPDJoint(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"N\",},{M[8],M[0],T\"is_active_c5_EulerAugPDJoint\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c5_EulerAugPDJoint_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc5_EulerAugPDJointInstanceStruct *chartInstance;
    chartInstance = (SFc5_EulerAugPDJointInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c5_EulerAugPDJoint(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc5_EulerAugPDJointInstanceStruct*)
    chartInstanceVar)->S,0);
  initialize_params_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*)
    chartInstanceVar);
  initialize_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c5_EulerAugPDJoint(void *chartInstanceVar)
{
  enable_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_disable_c5_EulerAugPDJoint(void *chartInstanceVar)
{
  disable_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c5_EulerAugPDJoint(void *chartInstanceVar)
{
  sf_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*) chartInstanceVar);
}

static mxArray* sf_internal_get_sim_state_c5_EulerAugPDJoint(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c5_EulerAugPDJoint
    ((SFc5_EulerAugPDJointInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = sf_get_sim_state_info_c5_EulerAugPDJoint();/* state var info */
...
static void sf_internal_set_sim_state_c5_EulerAugPDJoint(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  prhs[3] = (mxArray*) sf_get_sim_state_info_c5_EulerAugPDJoint();/* state var info */
  mxError = sf_mex_call_matlab(1, plhs, 4, prhs, "sfprivate");
  mxDestroyArray(prhs[0]);
  mxDestroyArray(prhs[1]);
  mxDestroyArray(prhs[2]);
  ...
  set_sim_state_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static mxArray* sf_opaque_get_sim_state_c5_EulerAugPDJoint(SimStruct* S)
{
  return sf_internal_get_sim_state_c5_EulerAugPDJoint(S);
}

static void sf_opaque_set_sim_state_c5_EulerAugPDJoint(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c5_EulerAugPDJoint(S, st);
}

static void sf_opaque_terminate_c5_EulerAugPDJoint(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc5_EulerAugPDJointInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c5_EulerAugPDJoint((SFc5_EulerAugPDJointInstanceStruct*)
      chartInstanceVar);
...
{
  compInitSubchartSimstructsFcn_c5_EulerAugPDJoint
    ((SFc5_EulerAugPDJointInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c5_EulerAugPDJoint(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c5_EulerAugPDJoint(SimStruct *S)
{
  ...
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"EulerAugPDJoint","EulerAugPDJoint",5);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"EulerAugPDJoint","EulerAugPDJoint",5,
                "RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"EulerAugPDJoint",
      "EulerAugPDJoint",5,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"EulerAugPDJoint","EulerAugPDJoint",
        5,1);
      sf_mark_chart_reusable_outputs(S,"EulerAugPDJoint","EulerAugPDJoint",5,1);
    }

    sf_set_rtw_dwork_info(S,"EulerAugPDJoint","EulerAugPDJoint",5);
...
static void mdlRTW_c5_EulerAugPDJoint(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "EulerAugPDJoint", "EulerAugPDJoint",5);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c5_EulerAugPDJoint(SimStruct *S)
{
  SFc5_EulerAugPDJointInstanceStruct *chartInstance;
  chartInstance = (SFc5_EulerAugPDJointInstanceStruct *)malloc(sizeof
    (SFc5_EulerAugPDJointInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc5_EulerAugPDJointInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c5_EulerAugPDJoint;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c5_EulerAugPDJoint;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c5_EulerAugPDJoint;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c5_EulerAugPDJoint;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c5_EulerAugPDJoint;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c5_EulerAugPDJoint;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c5_EulerAugPDJoint;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c5_EulerAugPDJoint;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c5_EulerAugPDJoint;
  chartInstance->chartInfo.mdlStart = mdlStart_c5_EulerAugPDJoint;
  chartInstance->chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c5_EulerAugPDJoint;
...
void c5_EulerAugPDJoint_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c5_EulerAugPDJoint(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c5_EulerAugPDJoint(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c5_EulerAugPDJoint(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c5_EulerAugPDJoint_method_dispatcher.\n"
