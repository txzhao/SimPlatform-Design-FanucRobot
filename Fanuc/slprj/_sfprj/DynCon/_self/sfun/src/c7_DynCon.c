...
static void initialize_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void initialize_params_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void enable_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void disable_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void c7_update_debugger_state_c7_DynCon(SFc7_DynConInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c7_DynCon(SFc7_DynConInstanceStruct
  *chartInstance);
static void set_sim_state_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance,
  const mxArray *c7_st);
static void finalize_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void sf_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void c7_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance);
static void compInitSubchartSimstructsFcn_c7_DynCon(SFc7_DynConInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c7_machineNumber, uint32_T
  c7_chartNumber);
static void c7_spat_Jacobian(SFc7_DynConInstanceStruct *chartInstance, real_T
  c7_in1[9], real_T c7_in2[6], real_T c7_b_spat_Jacobian[36]);
static void c7_eml_scalar_eg(SFc7_DynConInstanceStruct *chartInstance);
static void c7_b_eml_scalar_eg(SFc7_DynConInstanceStruct *chartInstance);
static void c7_fwd_kin(SFc7_DynConInstanceStruct *chartInstance, real_T c7_in1[9],
...
static void c7_emlrt_marshallIn(SFc7_DynConInstanceStruct *chartInstance, const
  mxArray *c7_EndVel, const char_T *c7_name, real_T c7_y[3]);
static uint8_T c7_b_emlrt_marshallIn(SFc7_DynConInstanceStruct *chartInstance,
  const mxArray *c7_b_is_active_c7_DynCon, const char_T *c7_name);
static void init_dsm_address_info(SFc7_DynConInstanceStruct *chartInstance);

/* Function Definitions */
static void initialize_c7_DynCon(SFc7_DynConInstanceStruct *chartInstance)
{
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  chartInstance->c7_is_active_c7_DynCon = 0U;
}
  ...
  *chartInstance)
{
  ...
  c7_b_hoistedGlobal = chartInstance->c7_is_active_c7_DynCon;
  ...
  const mxArray *c7_st)
{
  ...
  chartInstance->c7_is_active_c7_DynCon = c7_b_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c7_u, 1)),
    "is_active_c7_DynCon");
  sf_mex_destroy(&c7_u);
  c7_update_debugger_state_c7_DynCon(chartInstance);
...
static void c7_spat_Jacobian(SFc7_DynConInstanceStruct *chartInstance, real_T
  c7_in1[9], real_T c7_in2[6], real_T c7_b_spat_Jacobian
  [36])
{
...
static const mxArray *c7_sf_marshall(void *chartInstanceVoid, void *c7_u)
{
  ...
  real_T c7_c_u[3];
  const mxArray *c7_b_y = NULL;
  SFc7_DynConInstanceStruct *chartInstance;
  chartInstance = (SFc7_DynConInstanceStruct *)chartInstanceVoid;
  c7_y = NULL;
...
const mxArray *sf_c7_DynCon_get_eml_resolved_functions_info(void)
{
...
static void c7_emlrt_marshallIn(SFc7_DynConInstanceStruct *chartInstance, const
  mxArray *c7_EndVel, const char_T *c7_name, real_T
  c7_y[3])
{
  real_T c7_dv3[3];
  int32_T c7_i56;
...
static uint8_T c7_b_emlrt_marshallIn(SFc7_DynConInstanceStruct *chartInstance,
  const mxArray *c7_b_is_active_c7_DynCon, const char_T
  *c7_name)
{
  uint8_T c7_y;
  uint8_T c7_u0;
  sf_mex_import(c7_name, sf_mex_dup(c7_b_is_active_c7_DynCon), &c7_u0, 1, 3, 0U,
                0, 0U, 0);
  c7_y = c7_u0;
  sf_mex_destroy(&c7_b_is_active_c7_DynCon);
  return c7_y;
}
...
/* SFunction Glue Code */
void sf_c7_DynCon_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c7_DynCon_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c7_DynCon(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"EndVel\",},{M[8],M[0],T\"is_active_c7_DynCon\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c7_DynCon_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc7_DynConInstanceStruct *chartInstance;
    chartInstance = (SFc7_DynConInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c7_DynCon(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc7_DynConInstanceStruct*) chartInstanceVar)->S,
    0);
  initialize_params_c7_DynCon((SFc7_DynConInstanceStruct*) chartInstanceVar);
  initialize_c7_DynCon((SFc7_DynConInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c7_DynCon(void *chartInstanceVar)
{
  enable_c7_DynCon((SFc7_DynConInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c7_DynCon(void *chartInstanceVar)
{
  disable_c7_DynCon((SFc7_DynConInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c7_DynCon(void *chartInstanceVar)
{
  sf_c7_DynCon((SFc7_DynConInstanceStruct*) chartInstanceVar);
}

static mxArray* sf_internal_get_sim_state_c7_DynCon(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c7_DynCon((SFc7_DynConInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = sf_get_sim_state_info_c7_DynCon();/* state var info */
...
static void sf_internal_set_sim_state_c7_DynCon(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  prhs[3] = (mxArray*) sf_get_sim_state_info_c7_DynCon();/* state var info */
  mxError = sf_mex_call_matlab(1, plhs, 4, prhs, "sfprivate");
  mxDestroyArray(prhs[0]);
  mxDestroyArray(prhs[1]);
  mxDestroyArray(prhs[2]);
  ...
  set_sim_state_c7_DynCon((SFc7_DynConInstanceStruct*)chartInfo->chartInstance,
    mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static mxArray* sf_opaque_get_sim_state_c7_DynCon(SimStruct* S)
{
  return sf_internal_get_sim_state_c7_DynCon(S);
}

static void sf_opaque_set_sim_state_c7_DynCon(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c7_DynCon(S, st);
}

static void sf_opaque_terminate_c7_DynCon(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc7_DynConInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c7_DynCon((SFc7_DynConInstanceStruct*) chartInstanceVar);
...
{
  compInitSubchartSimstructsFcn_c7_DynCon((SFc7_DynConInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c7_DynCon(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c7_DynCon((SFc7_DynConInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c7_DynCon(SimStruct *S)
{
  ...
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"DynCon","DynCon",7);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"DynCon","DynCon",7,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"DynCon","DynCon",7,
      "gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"DynCon","DynCon",7,2);
      sf_mark_chart_reusable_outputs(S,"DynCon","DynCon",7,1);
    }

    sf_set_rtw_dwork_info(S,"DynCon","DynCon",7);
...
static void mdlRTW_c7_DynCon(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "DynCon", "DynCon",7);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c7_DynCon(SimStruct *S)
{
  SFc7_DynConInstanceStruct *chartInstance;
  chartInstance = (SFc7_DynConInstanceStruct *)malloc(sizeof
    (SFc7_DynConInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc7_DynConInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c7_DynCon;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c7_DynCon;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c7_DynCon;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c7_DynCon;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c7_DynCon;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c7_DynCon;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c7_DynCon;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c7_DynCon;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c7_DynCon;
  chartInstance->chartInfo.mdlStart = mdlStart_c7_DynCon;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c7_DynCon;
...
void c7_DynCon_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c7_DynCon(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c7_DynCon(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c7_DynCon(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c7_DynCon_method_dispatcher.\n"
