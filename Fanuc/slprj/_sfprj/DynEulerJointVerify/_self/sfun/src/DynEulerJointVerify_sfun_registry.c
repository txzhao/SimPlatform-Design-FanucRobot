...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_DynEulerJointVerify_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_DynEulerJointVerify_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_DynEulerJointVerify_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_DynEulerJointVerify_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_DynEulerJointVerify_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "DynEulerJointVerify") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_DynEulerJointVerify_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_DynEulerJointVerify_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"DynEulerJointVerify")) {
    return(sf_DynEulerJointVerify_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void DynEulerJointVerify_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    DynEulerJointVerify_terminator();
    sf_debug_terminate();
  }
...
extern void DynEulerJointVerify_initializer(void);
extern void DynEulerJointVerify_register_exported_symbols(SimStruct* S);
extern void DynEulerJointVerify_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  DynEulerJointVerify_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      DynEulerJointVerify_debug_initialize();
    }

    DynEulerJointVerify_initializer();
  }
